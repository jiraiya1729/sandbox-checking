...
if (!DAYTONA_API_KEY || !DAYTONA_API_URL) {
  throw new Error('Missing Daytona env vars: DAYTONA_API_KEY or DAYTONA_API_URL');
}
...
function getSandboxIdAndPortFromPath(fullPath: string[]): { sandboxId: string; port: number } {
  console.log('=== PATH DEBUG === Full path segments:', fullPath);
  ...
  console.log('=== PATH DEBUG === Extracted sandboxId:', sandboxId, 'portStr:', portStr);
  const port = parseInt(portStr, 10);
  if (isNaN(port)) {
    console.error('=== PATH ERROR === Invalid port (NaN):', portStr);
    throw new Error('Invalid port');
  }
  console.log('=== PATH DEBUG === Valid port:', port);
...
export async function GET(req: NextRequest) {
  console.log('=== ROUTE HIT === GET /api/proxy/preview/...');
  return handleProxy(req);
}

export async function POST(req: NextRequest) {
  console.log('=== ROUTE HIT === POST /api/proxy/preview/...');
  return handleProxy(req);
}
...
async function handleProxy(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.log('=== PROXY DEBUG === Full target URL:', fullTargetUrl);
    ...
    const isBufferable = contentType.includes('text/html') || contentType.includes('text/css');
    ...
    if (isBufferable && bodyPreview) {
      ...
      cleanHeaders.set('content-length', rewrittenBody.length.toString());
